...
function deletetask(name:string,project:string){
    axios({url: '/api/task',data:{name,project}, method:"DELETE"}).then((res) => {
        console.log(res);
    }).catch((err) => {
        console.log(err.response.data);
    });
...
export default function Task() {
    ...
    return (
        <div className="taskContainer">
            ...
            <label>Project name</label>
            <input type="text" onChange={(e) => setProject(e.currentTarget.value)} value={project} />
            <button onClick={() => deletetask(name,project)}>delete</button>
